...
/** ===== Mini-Chart-Komponenten (SVG, keine Libs) ===== */
function Sparkline({ values = [], width = 280, height = 60, stroke = ACCENT }) {
  if (!values.length) return null;
  const max = 100; // Prozent
  const min = 0;
...
function Donut({ value = 0, size = 120, stroke = ACCENT, bg = "#334155" }) {
  const r = size / 2 - 10;
  const c = 2 * Math.PI * r;
  ...
  return (
    <svg width={size} height={size}>
      <g transform={`translate(${size / 2}, ${size / 2})`}>
        <circle r={r} fill="none" stroke={bg} strokeWidth="12" />
        <circle
          r={r}
          fill="none"
          stroke={stroke}
  ...
  return (
    <svg width={width} height={height}>
      <g transform={`translate(${pad},${pad})`}>
        {/* Achse */}
        <line x1="0" y1={h} x2={w} y2={h} stroke={axisColor} strokeWidth="1" />
        {/* Bars */}
        {rows.map((r, i) => {
          const x = i * (barW + 8);
          ...
          return (
            <g key={r.label} transform={`translate(${x},0)`}>
              <rect x="0" y={y} width={barW} height={barH} fill={barColor} />
              <text
                x={barW / 2}
                y={h + 14}
                textAnchor="middle"
...
function QuestionCard({ index, q, selected, onSelect, showResult }) {
  ...
  return (
    ...
    >
      <div
        style={{ display: "flex", alignItems: "center", gap: 10, color: "#D1D5DB" }}
      >
        <div
          style={{
            width: 28,
...
function AdminEditor({ editing, onCancel, onSaved }) {
  ...
  const [opts, setOpts] = useState(() => {
    ...
    return [...base, ...range(Math.max(0, 4 - base.length)).map(() => "")].slice(
      0,
      4
    );
  });
  ...
  return (
    ...
    >
      <div style={{ fontWeight: 700, color: "#E5E7EB" }}>
        {isEdit ? "Frage bearbeiten" : "Neue Frage anlegen"}
      </div>
      <input
...
export default function App() {
  ...
  async function fetchQuestions() {
    ...
    try {
      ...
      const pool =
        data?.filter(
          (r) => Array.isArray(r.options) && typeof r.correct_idx === "number"
        ) || [];
  ...
  async function saveAttempt() {
    ...
    try {
      ...
      const payload = {
        ...
        started_at: new Date(now - 1000 * 30).toISOString(), // grobe Dauer (30s) – optional ersetzen
  ...
  async function loadMyStats() {
    ...
    try {
      ...
      const avg = count
        ? Math.round(
            (data.reduce((s, a) => s + a.score / a.total, 0) / count) * 100
          )
        : 0;
      const best = count
        ? Math.max(...data.map((a) => Math.round((a.score / a.total) * 100)))
        : 0;
    ...
    try {
      ...
      const rows = Array.from(byEmail.entries()).map(([email, arr]) => {
        ...
        const avg = Math.round(
          (arr.reduce((s, x) => s + x.score / x.total, 0) / count) * 100
        );
        const best = Math.max(
          ...arr.map((x) => Math.round((x.score / x.total) * 100))
        );
  ...
  const header = (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <Sun />
        <div style={{ fontWeight: 800, fontSize: 20, color: "#E5E7EB" }}>
          MaQuiz
        </div>
      </div>
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        {user && (
          <span style={{ color: "#9CA3AF", fontSize: 14 }}>
            Angemeldet als{" "}
            <b style={{ color: "#E5E7EB" }}>{user.email}</b>
            {role === "admin" ? " · Admin" : ""}
          </span>
        )}
        {user ? (
  ...
  return (
    ...
    >
      ...
      >
            ...
            {tab === "quiz" &&
              (loading
                ...
                : `${questions.length} Fragen geladen${
                    showResult ? ` · Ergebnis: ${score}/${questions.length}` : ""
                  }`)}
            {tab === "stats" && (statsLoading ? "Lade Statistik…" : "")}
            {tab === "admin" &&
              (adminLoading ? "Lade…" : `${adminList.length} Fragen`)}
          </div>
        </div>
        ...
        {tab === "quiz" ? (
          <>
            ...
            {!showResult ? (
              ...
              >
                <h2 style={{ margin: "4px 0 12px 0", color: "#F3F4F6" }}>
                  Auswertung
                </h2>
                <div
                  style={{
          ...
          <div style={{ display: "grid", gap: 14 }}>
            ...
            >
              ...
              >
                ...
                {/* Donut letztes Ergebnis */}
                <Donut
                  value={
                    myAttempts.length
                      ? Math.round(
                          (myAttempts[myAttempts.length - 1].score /
                            myAttempts[myAttempts.length - 1].total) *
                            100
                        )
                      : 0
                  }
              ...
              >
                ...
                >
                  <div style={{ color: "#E5E7EB", marginBottom: 6, fontWeight: 600 }}>
                    Verlauf (%)
                  </div>
                  <Sparkline
                    values={(myAttempts || []).map((a) =>
                ...
                <div style={{ display: "grid", gap: 6 }}>
                  ...
                  {myAttempts.length === 0 && (
                    <div style={{ color: "#9CA3AF" }}>Noch keine Versuche.</div>
                  )}
                </div>
              </div>
            </div>
            ...
            {role === "admin" && (
              ...
              >
                ...
                >
                  <div style={{ color: "#E5E7EB", marginBottom: 6, fontWeight: 600 }}>
                    ⌀ Durchschnitt pro Mitarbeiter (Top 10)
                  </div>
                  <Bars
                    rows={teamAgg
                      .slice(0, 10)
                      .map((r) => ({ label: (r.email || "").split("@")[0], value: r.avg }))}
                    width={640}
                    height={240}
                  />
                </div>
                ...
                <div style={{ display: "grid", gap: 6 }}>
                  {(teamAgg || []).map((r) => (
                    <div
                      ...
                      style={{
                        display: "grid",
                        gridTemplateColumns: "1.2fr .5fr .5fr .8fr",
                        gap: 8,
                        background: "#0B1220",
                        border: `1px solid ${HILITE}`,
                        borderRadius: 8,
          ...
          <div style={{ display: "grid", gap: 14 }}>
            ...
            >
              ...
              <div style={{ display: "grid", gap: 8 }}>
                ...
                {adminList.length === 0 && (
                  <div style={{ color: "#9CA3AF" }}>Keine Fragen gefunden.</div>
                )}
              </div>
            </div>
          </div>
        ...
        {authMsg && (
          <p style={{ color: "#F87171", whiteSpace: "pre-wrap" }}>{authMsg}</p>
        )}
      </div>
    </div>
  );
